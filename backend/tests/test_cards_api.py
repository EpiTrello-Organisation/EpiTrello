...
def _setup_board_and_list(client, headers):
    ...
    list_resp = client.post(
        f"/api/lists/?board_id={board_id}", json={"title": "List"}, headers=headers
    )
...
class TestCreateCard:
    ...
    def test_create_card_auto_position(self, client):
        ...
        client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "A"}, headers=headers
        )
        resp = client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "B"}, headers=headers
        )
    ...
    def test_create_card_list_not_found(self, client):
        ...
        resp = client.post(
            f"/api/cards/?list_id={fake_id}", json={"title": "X"}, headers=headers
        )
...
class TestListCards:
    ...
    def test_list_cards_returns_ordered(self, client):
        ...
        client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "First"}, headers=headers
        )
        client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "Second"}, headers=headers
        )
...
class TestUpdateCard:
    def test_update_card_title(self, client):
        ...
        create_resp = client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "Old"}, headers=headers
        )
        card_id = create_resp.json()["id"]

        resp = client.put(
            f"/api/cards/{card_id}", json={"title": "New"}, headers=headers
        )
        assert resp.status_code == 200
    ...
    def test_update_card_description(self, client):
        ...
        _, list_id = _setup_board_and_list(client, headers)
        create_resp = client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "C"}, headers=headers
        )
        card_id = create_resp.json()["id"]

        resp = client.put(
            f"/api/cards/{card_id}", json={"description": "Updated"}, headers=headers
        )
        assert resp.status_code == 200
    ...
    def test_update_card_position(self, client):
        ...
        resp = client.put(
            f"/api/cards/{card_id}", json={"position": 10}, headers=headers
        )
        assert resp.status_code == 200
    ...
    def test_update_card_not_member(self, client):
        ...
        _, list_id = _setup_board_and_list(client, headers_alice)
        create_resp = client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "C"}, headers=headers_alice
        )
        card_id = create_resp.json()["id"]
        ...
        resp = client.put(
            f"/api/cards/{card_id}", json={"title": "X"}, headers=headers_bob
        )
        assert resp.status_code == 403
...
class TestDeleteCard:
    def test_delete_card_success(self, client):
        ...
        _, list_id = _setup_board_and_list(client, headers)
        create_resp = client.post(
            f"/api/cards/?list_id={list_id}", json={"title": "Bye"}, headers=headers
        )
        card_id = create_resp.json()["id"]
